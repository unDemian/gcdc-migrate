<section class="block clearfix clear">
	<h3><?=($task['type'] == 'clean' ? ucfirst($task['type']) . 'ed' : ucfirst($task['type']) . 'd')?> Data</h3>

	<!-- HEADING -->
	<hr class="no-margin-top"/>
	<? if(isset($migratedData['playlists'])): ?>
		<div class="col-xs-4 no-padding"><i>Cleaned Playlists: <strong><?=count($migratedData['playlists'])?></strong></i></div>
	<? endif; ?>

	<? if(isset($migratedData['subscriptions'])): ?>
		<div class="col-xs-4 no-padding"><i>Cleaned Subscriptions: <strong><?=count($migratedData['subscriptions'])?></strong></i></div>
	<? endif; ?>
	<br />
	<hr class="clear"/>
