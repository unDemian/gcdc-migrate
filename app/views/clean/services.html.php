	...
	<? if($services): ?>
		...
		<? if($has): ?>
			<div class="clear clearfix">
				...
				<p>Please select which one of the following service's data you want to clean:</p> <br />
				<ul class="services">
					...
					<? foreach($services as $key => $service): ?>
						<? if($service['sync']): ?>
							<?
								$has = true;
								$selected = 'select';
								if( $_SESSION['clean']['service']) {
									if($service['id'] == $_SESSION['clean']['service']) {
										$selected = 'selected';
									}
								}
							?>
		...
		<? else: ?>
			<div class="alert alert-info text-center alert-dismissable no-common">
				You have no services enabled that support cleaning. Please refine your permissions <a href="<?=Render::link('accounts/permissions')?>">here</a>.
			</div>
		<? endif; ?>
	<? else: ?>
		<div class="alert alert-info text-center alert-dismissable no-common">
